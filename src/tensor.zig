...
// Activation quantization fast-path (Q8_0 weights × Q8_0 activations).
pub const USE_Q8_ACTIVATIONS: bool = true;

// LUT-based multiplication: mengganti hardware multiply dengan table lookup
pub const USE_LUT_MUL: bool = true;

// Batasi ukuran cols yang boleh pakai activation-quant buffer stack.
pub const QIN_MAX_COLS: usize = 16384;
pub const QIN_MAX_BLOCKS: usize = QIN_MAX_COLS / 32;

// Default: jangan pakai qin untuk cols kecil (mis. dim=2048) karena sering kalah dari float-SIMD.
pub const QIN_MIN_COLS: usize = 4096;
pub const QIN_SMALLCOLS_MIN_ROWS: usize = 65536;

// Heuristic ST: gunakan qin hanya kalau rows besar.
...
// ============================================================
// Optional ISA dispatch + runtime CPU feature detection (x86_64)
// ============================================================
...
// chosen at init (auto-tuned if not forced)
var g_use_fma: bool = false;

// qin small-cols autotune (cols kecil seperti 2048, untuk logits)
var g_use_qin_smallcols: bool = false;
var g_force_qin_smallcols: ?bool = null;

// ============================================================
// Public lifecycle (production-grade)
// ============================================================

/// Call once at program start.
/// Safe to call multiple times; will re-evaluate env overrides and (re)autotune.
pub fn initIsa(allocator: std.mem.Allocator) void {
    ...
    // QIN small-cols override: "0" => false, selain itu true
...
/// Production cleanup: stop worker threads and free pool memory.
...
fn chooseQinSmallCols() bool {
    ...
    for (0..iters) |_| {
        ...
        while (b < blocks) : (b += 1) {
            ...
            if (g_use_fma) {
                accf += dotQ8Block_vec_fma(p, v0, v1, v2, v3);
            } else {
                accf += dotQ8Block_vec(p, v0, v1, v2, v3);
            }
...
/// Raw SIMD i8x32 dot product — always uses hardware multiply.
/// Used internally by chooseQinSmallCols autotune (must not dispatch to LUT).
...
/// i8x32 dot product — dispatches to LUT when enabled.
...
/// i8x32 dot product with pre-loaded x8 vector — dispatches to LUT when enabled.
inline fn dotI8x32_prex(w_ptr: [*]const u8, x8: VI8x32) i32 {
    if (USE_LUT_MUL and lut.g_lut_enabled) {
        // LUT needs raw pointer, store vector to stack
...
/// LUT path: more aggressive qin usage — quantize input is always beneficial
/// because LUT dot product works best with pre-quantized i8 data
inline fn canUseLutQin(rows: usize, cols: usize) bool {
    if (cols > QIN_MAX_COLS) return false;
    if ((cols % 32) != 0) return false;
    _ = rows;
    return true; // LUT always benefits from pre-quantized input
}
...
fn quantizeInputQ8_0(q_out: []i8, s_out: []f32, input: []const f32, cols: usize) void {
    ...
    while (b < blocks) : (b += 1) {
        ...
        i = 0;
        while (i < QBLOCK) : (i += 1) {
            const v = input[base + i] * inv;
            var qi: i32 = @intFromFloat(@round(v));
            if (qi > 127) qi = 127;
            if (qi < -127) qi = -127;
            q_out[base + i] = @intCast(qi);
        }
    }
}
        ...
        .Q8_0 => {
            ...
            if (USE_LUT_MUL and lut.g_lut_enabled) {
                if (canUseLutQin(rows, cols)) {
                    ...
                    const qbuf = qbuf_storage[0..cols];
                    const scales = scales_storage[0..blocks];

                    quantizeInputQ8_0(qbuf, scales, input, cols);
                    lut.matVecQ8_0_lut_qin(out, weight.data, qbuf, scales, rows, cols);
                } else {
...
// ============================================================
// Persistent pool (production-grade)
// ============================================================
...
const MatVecPool = struct {
            ...
            if (start < end) {
                switch (kind) {
                    ...
                    .pair => {
                        if (USE_LUT_MUL and lut.g_lut_enabled) {
                            if (qin_enabled and cols == qin_cols) {
                                lut.matVecQ8_0_range_pair_lut_qin(
                                    out[start..end],
                                    data,
                                    out_b[start..end],
                                    data_b,
                                ...
                                lut.matVecQ8_0_range_pair_lut_f32(
                                    out[start..end],
                                    data,
                                    out_b[start..end],
                                    data_b,
                            ...
                            if (qin_enabled and cols == qin_cols) {
                                matVecQ8_0_range_pair_batched2_qin_prequant(
                                    out[start..end],
                                    data,
                                    out_b[start..end],
                                    data_b,
                                ...
                                matVecQ8_0_range_pair_batched2_f32(
                                    out[start..end],
                                    data,
                                    out_b[start..end],
                                    data_b,
    ...
    fn publishSingle(pool: *MatVecPool, out: []f32, data: []const u8, input: []const f32, rows: usize, cols: usize, active_threads: usize) void {
        ...
        // Determine qin strategy based on LUT mode
        const use_qin = if (USE_LUT_MUL and lut.g_lut_enabled)
            canUseLutQin(rows, cols)
        else
            canUseQinRC(rows, cols);
    ...
    pub fn runQ8Single(pool: *MatVecPool, out: []f32, data: []const u8, input: []const f32, rows: usize, cols: usize, active_threads: usize) void {
        ...
        const active = active_threads;
        const chunk = (rows + active - 1) / active;
        const end0 = @min(rows, chunk);
        if (0 < end0) {
            if (USE_LUT_MUL and lut.g_lut_enabled) {
                if (pool.qin_enabled) {
                    const qbuf: []const i8 = pool.qin_qbuf[0..cols];
                    const scales_slice: []const f32 = pool.qin_scales[0..(cols / 32)];
                    lut.matVecQ8_0_range_lut_qin(out[0..end0], data, qbuf, scales_slice, 0, end0, cols);
                } else {
                ...
                if (pool.qin_enabled) {
                    const qbuf: []const i8 = pool.qin_qbuf[0..cols];
                    const scales_slice: []const f32 = pool.qin_scales[0..(cols / 32)];
                    matVecQ8_0_range_batched8_qin_prequant(out[0..end0], data, qbuf, scales_slice, 0, end0, cols);
                } else {
        ...
        const target: u32 = @intCast(active - 1);

        pool.mutex.lock();
        while (pool.done.load(.acquire) != target) {
            pool.done_cv.wait(&pool.mutex);
        }
        pool.mutex.unlock();
    }
        ...
        if (0 < end0) {
            if (USE_LUT_MUL and lut.g_lut_enabled) {
                if (pool.qin_enabled) {
                    const qbuf: []const i8 = pool.qin_qbuf[0..cols];
                    const scales_slice: []const f32 = pool.qin_scales[0..(cols / 32)];
                    lut.matVecQ8_0_range_pair_lut_qin(out_a[0..end0], data_a, out_b[0..end0], data_b, qbuf, scales_slice, 0, end0, cols);
                } else {
                ...
                if (pool.qin_enabled) {
                    const qbuf: []const i8 = pool.qin_qbuf[0..cols];
                    const scales_slice: []const f32 = pool.qin_scales[0..(cols / 32)];
                    matVecQ8_0_range_pair_batched2_qin_prequant(out_a[0..end0], data_a, out_b[0..end0], data_b, qbuf, scales_slice, 0, end0, cols);
                } else {
...
// ============================================================
// Q8_0 kernels (scalar + SIMD float + SIMD qin) — ORIGINAL paths
// ============================================================
...
fn matVecQ8_0_simd_f32(out: []f32, data: []const u8, input: []const f32, rows: usize, cols: usize) void {
    ...
    while (row < rows) : (row += 1) {
        const row_ptr = data.ptr + row * row_stride;
...
fn matVecQ8_0_simd_qin(out: []f32, data: []const u8, input: []const f32, rows: usize, cols: usize) void {
    ...
    while (r + 8 <= rows) : (r += 8) {
        ...
        while (b < blocks_per_row) : (b += 1) {
            ...
            const d0: i32 = dotI8x32_prex(p0 + 2, x8);
            const d1: i32 = dotI8x32_prex(p1 + 2, x8);
            const d2: i32 = dotI8x32_prex(p2 + 2, x8);
            const d3: i32 = dotI8x32_prex(p3 + 2, x8);
            const d4: i32 = dotI8x32_prex(p4 + 2, x8);
            const d5: i32 = dotI8x32_prex(p5 + 2, x8);
            const d6: i32 = dotI8x32_prex(p6 + 2, x8);
            const d7: i32 = dotI8x32_prex(p7 + 2, x8);
    ...
    while (r < rows) : (r += 1) {
        ...
        while (b < blocks_per_row) : (b += 1) {
            ...
            const sx = scales[b];
            const x8: VI8x32 = loadI8x32(qbuf.ptr + b * QBLOCK);
            const dot: i32 = dotI8x32_prex(w + 2, x8);
...
fn matVecQ8_0_range_batched8_f32(
    out_range: []f32,
    data: []const u8,
    input: []const f32,
    row_start: usize,
...
fn matVecQ8_0_range_batched8_qin_prequant(
    out_range: []f32,
    data: []const u8,
    qbuf: []const i8,
    scales: []const f32,
    ...
    while (r < nrows) : (r += 1) {
        ...
        while (b < blocks_per_row) : (b += 1) {
            ...
            const sw = readF16(p);
            const dot: i32 = dotI8x32_prex(p + 2, x8);
            sum += (sw * sx) * @as(f32, @floatFromInt(dot));
...
fn matVecQ8_0_range_pair_batched2_f32(
    out_a_range: []f32,
    data_a: []const u8,
    out_b_range: []f32,
    data_b: []const u8,
    ...
    while (r + 2 <= nrows) : (r += 2) {
        ...
        var acc_a0: SimdF32 = @splat(0);
        var acc_b0: SimdF32 = @splat(0);
        var acc_a1: SimdF32 = @splat(0);
        var acc_b1: SimdF32 = @splat(0);
        ...
        if (use_fma) {
            while (b < blocks_per_row) : (b += 1) {
                if (b + PREFETCH_AHEAD < blocks_per_row) {
                    const off = PREFETCH_AHEAD * QBYTES;
                    @prefetch(pa0 + off, .{ .rw = .read, .locality = 3, .cache = .data });
                    @prefetch(pb0 + off, .{ .rw = .read, .locality = 3, .cache = .data });
                    @prefetch(pa1 + off, .{ .rw = .read, .locality = 3, .cache = .data });
                    @prefetch(pb1 + off, .{ .rw = .read, .locality = 3, .cache = .data });
                }
                ...
                acc_a0 += dotQ8Block_vec_fma(pa0, v0, v1, v2, v3);
                acc_b0 += dotQ8Block_vec_fma(pb0, v0, v1, v2, v3);
                acc_a1 += dotQ8Block_vec_fma(pa1, v0, v1, v2, v3);
                acc_b1 += dotQ8Block_vec_fma(pb1, v0, v1, v2, v3);
            ...
            while (b < blocks_per_row) : (b += 1) {
                ...
                acc_a0 += dotQ8Block_vec(pa0, v0, v1, v2, v3);
                acc_b0 += dotQ8Block_vec(pb0, v0, v1, v2, v3);
                acc_a1 += dotQ8Block_vec(pa1, v0, v1, v2, v3);
                acc_b1 += dotQ8Block_vec(pb1, v0, v1, v2, v3);
        ...
        out_a_range[r + 0] = @reduce(.Add, acc_a0);
        out_b_range[r + 0] = @reduce(.Add, acc_b0);
        out_a_range[r + 1] = @reduce(.Add, acc_a1);
        out_b_range[r + 1] = @reduce(.Add, acc_b1);
    }
    ...
    while (r < nrows) : (r += 1) {
        ...
        var acc_a: SimdF32 = @splat(0);
        var acc_b: SimdF32 = @splat(0);
        ...
        if (use_fma) {
            while (b < blocks_per_row) : (b += 1) {
                ...
                acc_a += dotQ8Block_vec_fma(pa, v0, v1, v2, v3);
                acc_b += dotQ8Block_vec_fma(pb, v0, v1, v2, v3);
            ...
            while (b < blocks_per_row) : (b += 1) {
                ...
                acc_a += dotQ8Block_vec(pa, v0, v1, v2, v3);
                acc_b += dotQ8Block_vec(pb, v0, v1, v2, v3);
        ...
        out_a_range[r] = @reduce(.Add, acc_a);
        out_b_range[r] = @reduce(.Add, acc_b);
    }
}

fn matVecQ8_0_range_pair_batched2_qin_prequant(
    out_a_range: []f32,
    data_a: []const u8,
    out_b_range: []f32,
    data_b: []const u8,
    ...
    while (r < nrows) : (r += 1) {
        ...
        while (b < blocks_per_row) : (b += 1) {
            ...
            const da: i32 = dotI8x32_prex(pa + 2, x8);
            const db: i32 = dotI8x32_prex(pb + 2, x8);
